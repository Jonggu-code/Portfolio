...
const Header = ({ activeSection, scrollToSection, sectionRefs }) => {
  ...
  return (
    ...
    >
      <div
        className="logo opacity-0 h-full fade-in delay-2 flex items-center lg:w-[210px]"
        onClick={moveTop}
      >
        <img
          className="w-[40px]"
      ...
      <div className="userMenu relative flex w-max lg:w-[210px]">
        {/* 햄버거 버튼 */}
        <div
          className={`hamberBtn transition-all duration-[0.3s] flex relative w-10 h-10 justify-center items-center transition-color opacity-100 text-[25px] rounded-[10px] cursor-pointer lg:opacity-0 lg:hidden ${isHamberActive ? (isDark ? "text-white" : "text-customDark") : isDark ? "text-white bg-customGray" : "bg-slate-200"} ${isDark ? "hover:bg-customGray text-customDark" : "hover:bg-slate-200 text-customGray"}`}
          onClick={toggleMenu}
        >
          ☰
        </div>
